...
class makeEquivTestCase(unittest.TestCase):
    ...
    def testResHistogram(self):
        s=Simulation()
        s.initialize()
        r = Resource(monitored = True, monitorType = Monitor,
                     name = 'TheResource / Monitor',sim=s)
        r.waitMon.setHistogram(high = 3, nbins = 3)
        r.actMon.setHistogram(high = 3, nbins = 3)

        for i in range (5):
          a = Actor(sim=s)
          s.activate(a, a.act(r))
        s.simulate(until = 20)
        ...
        s=Simulation()
        s.initialize()
        r = Resource(monitored = True, monitorType = Tally,
                     name = 'TheResource / Tally',sim=s)
        r.waitMon.setHistogram(high = 3, nbins = 3)
        r.actMon.setHistogram(high = 3, nbins = 3)
        for i in range (5):
          a = Actor(sim=s)
