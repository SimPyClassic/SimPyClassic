...
call the Classical style, uses user-defined Process objects with their
The simulation is started by creating one or more objects, activating
their Process Execution Methods and then, in a main block of the
program, calling the ``simulate()`` function to start the
simulation. This simpler style seems to be more acceptable to many users.
